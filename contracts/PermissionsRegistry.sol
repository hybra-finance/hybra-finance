...
contract PermissionsRegistry {
    ...
    event SetHybraTeamMultisig(address indexed multisig);
    event SetHybraMultisig(address indexed multisig);



    constructor() {
        hybraTeamMultisig = msg.sender;
        hybraMultisig = msg.sender;
    ...
    modifier onlyHybraMultisig() {
        require(msg.sender == hybraMultisig, "!hybraMultisig");
        _;
    }
    ...
    *************************************************************/
    ...
    function setEmergencyCouncil(address _new) external {
        require(msg.sender == emergencyCouncil || msg.sender == hybraMultisig, "not allowed");
    ...
    /// @param _new new address    
    function setHybraTeamMultisig(address _new) external {
        require(msg.sender == hybraTeamMultisig, "not allowed");
        require(_new != address(0), "addr 0");
        require(_new != hybraTeamMultisig, "same multisig");
        hybraTeamMultisig = _new;
        
        emit SetHybraTeamMultisig(_new);
    }

    /// @notice set black multisig
    /// @param _new new address    
    function setHybraMultisig(address _new) external {
        require(msg.sender == hybraMultisig, "not allowed");
        require(_new != address(0), "addr0");
        require(_new != hybraMultisig, "same multisig");
        hybraMultisig = _new;
        
        emit SetHybraMultisig(_new);
    }
